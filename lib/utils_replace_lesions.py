...
def fig_blend_lesion2(slice_healthy_inpain2, coords_big, GEN, decreasing_sequence, path_synthesis, len_lesion, file_path="images/for_gifs/synthesis.png", Tp=10, V_MAX=1, close=True, plot_size=6):
    # name_synthesis = path_synthesis.split('/')[-3]
    # name_synthesis_two_lines = '\n_scale'.join(name_synthesis.split('_seed')) # two lines
    fig, ax = plt.subplots(1,2, gridspec_kw={'width_ratios': [30, 1]}, figsize=(8,8));
...
def replace_with_nCA(path_synthesis, scan_slice, SCAN_NAME, SLICE, texture, mask_outer_ring = False, POST_PROCESS = True, blur_lesion = False, TRESH_PLOT=10):
    # scan_slice = scan
    ...
    for GEN in decreasing_sequence:
        ...
        pathlib.Path(path_gifs).mkdir(parents=True, exist_ok=True)
        scan_name_with_slice = f'{SCAN_NAME}_{SLICE}.gif'
        if scan_name_with_slice not in os.listdir(path_gifs):
            _ = fig_blend_lesion2(slice_healthy_inpain2, coords_big2, GEN, decreasing_sequence, path_synthesis, len(lesion), file_path=f"{path_gifs}synthesis.png", Tp=30, V_MAX=V_MAX)
